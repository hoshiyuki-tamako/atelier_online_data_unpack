...
export class TextureExport {
  public async process(sourceFolder: string, rootFolder: string) {
    ...
    return Promise.all(names.map((name) => this.processTexture2D(name, files, textureFolder, rootFolder)));
  }
  ...
  private async processTexture2D(name: string, files: string[], textureFolder: string, rootFolder: string) {
    ...
    return Promise.all(
      files.filter((p) => !p.includes('#') && regex.exec(p))
        .map((p) => fs.copy(path.join(textureFolder, p), path.join(outFolder, p)))
    );
  }
}
